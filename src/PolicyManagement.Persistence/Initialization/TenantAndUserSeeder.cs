...
{
    public static class TenantAndUserSeeder
    {
        // Fixed tenant IDs
        ...
        public static async Task SeedAsync(IServiceProvider services, ILogger logger)
        {
            ...
            logger.LogInformation("Seeding default tenants.");
            ...
            if (existingTenant1 != null)
                dbContext.Tenants.Remove(existingTenant1);

            if (existingTenant2 != null)
                dbContext.Tenants.Remove(existingTenant2);
            ...
            await SeedRolesAsync(services.GetRequiredService<RoleManager<ApplicationRole>>(), logger);
        ...
        {
            foreach (var roleName in Enum.GetNames(typeof(DefaultRoles)))
            {
                if (!await roleManager.RoleExistsAsync(roleName))
                {
                    logger.LogInformation("Creating role {RoleName}", roleName);
                    var applicationRole = new ApplicationRole { Name = roleName, NormalizedName = roleManager.NormalizeKey(roleName) };
