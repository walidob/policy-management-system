...
try {
    ...
    dotnet ef migrations add InitialMigration -c DefaultDbContext -o Migrations/DefaultDb -p $projectPath -s $startupProject
    
    Write-Host "Creating migration for TenantDbContextBase..." -ForegroundColor Green
    dotnet ef migrations add TenantDbInitialMigration -c TenantDbContextBase -o Migrations/Tenant -p $projectPath -s $startupProject
